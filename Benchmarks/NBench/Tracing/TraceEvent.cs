...
namespace NBench.Tracing
{
    ...
    public abstract class TraceMessage
    {
        ...
        {
            //return $"{TraceIndicator}[{Level.ToString().ToUpperInvariant()}][{Timestamp}][Thread {ThreadId.ToString().PadLeft(4, '0')}] {Message}";
            return $"[{Level.ToString().ToUpperInvariant()}][{Timestamp}] {Message}";
        }
    }
